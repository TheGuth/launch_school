...
# to a block that is provided in the method call. If the block returns a value of true
# for exactly one element, then #one? returns true. Otherwise, #one? returns false.
# Note in particular that #one? will stop searching the collection the second time
# the block returns true.

# Write a method called one? that behaves similarly for Arrays. 
# It should take an Array as an argument, and a block. 
# It should return true if the block returns true for exactly one of the element 
...
def one?(array)
  true_items = []
  array.each do |item|
    return false if true_items.size > 1
    if yield(item)
      true_items << yield(item)
    end
  end
  return false if true_items.empty?
  return true if true_items.size == 1
end

...
# Question 1. Hashes are commonly found to be more complex than the simple one we
# saw earlier:
...
munsters.each do |names, details|
  if munsters[names]['gender'] == 'male'
    total_male_ages += munsters[names]["age"]
  end
end

p total_male_ages

# Question 2. One of the most frequently used real-world string properties is that of 
#{ }"string substitution", where we take a hard-coded string and modify it with various 
...
munsters.each do |names, _|
  puts "#{names} is a #{munsters[names]['age']} year old #{munsters[names]['gender']}"
end
...
# Question 3. In an earlier exercise we saw that depending on variables to be modified
...
# Question 4. A quick glance at the docs for the Ruby String class will have you 
...
# Question 6. One day Spot was playing with the Munster family's home computer and 
...
# Did the family's data get ransacked, or did the mischief only mangle a local copy of
# the original hash? (why?)

# it messed with the local copy 

# Question 7. Method calls can take expressions as arguments. Suppose we define a 
# function called rps as follows, which follows the classic rules of rock-paper-scissors 
# game, but with a slight twist that it declares whatever hand was used in the tie as
#  the result of that tie.

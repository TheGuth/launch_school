...
# using the associated block to perform some sort of operation on each collection
# member. The returned values from the block are then built up into an Array that
# is then returned to the caller. Note in particular that every time #map yields 
# to the block, it obtains just one value. That value may be a complex value - it 
...
# #inject, loop, for, while, or until to iterate through the Array passed
# in as an argument, but must not use any other methods that iterate through 
